...
((reader 'put)
 '((define (require p) (if (not p) (amb)))
   ...
   (define (xor a b)
     (if a (not b) b))
   (define (solve)
     (let ((betti (amb 1 2 3 4 5))
           (etel (amb 1 2 3 4 5))
           (djoan (amb 1 2 3 4 5))
           (kitti (amb 1 2 3 4 5))
           (meri (amb 1 2 3 4 5)))
       (require (distinct? (list betti etel djoan kitti meri)))
       (require (xor (= kitti 2) (= betti 3)))
       (require (xor (= etel 1) (= djoan 2)))
       (require (xor (= djoan 3) (= etel 5)))
       (require (xor (= kitti 2) (= meri 4)))
       (require (xor (= meri 4) (= betti 1)))
       (list (list 'betti betti)
             (list 'etel etel)
             (list 'djoan djoan)
             (list 'kitti kitti)
             (list 'meri meri))))
